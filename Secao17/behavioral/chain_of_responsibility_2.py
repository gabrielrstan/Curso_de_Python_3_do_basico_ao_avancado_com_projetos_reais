...
class HandlerABC(Handler):
    def __init__(self, sucessor: Handler) -> None:
        super().__init__()
        self.letters = ['A', 'B', 'C']
        self.successor = sucessor

    def handle(self, letter: str) -> str:
        if letter in self.letters:
            return (f'{self.__class__.__name__} managed treat the letter, '
                    f'value - {letter}')
        return self.successor.handle(letter)


class HandlesDEF(Handler):
    def __init__(self, sucessor: Handler) -> None:
        super().__init__()
...
class HandlerUnsolved(Handler):
    def handle(self, letter: str) -> str:
        return (f'{self.__class__.__name__} could not treat the letter, '
                f'value - {letter}')


if __name__ == '__main__':
    handler_unsolved = HandlerUnsolved()
    handler_def = HandlesDEF(handler_unsolved)
